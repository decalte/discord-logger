...
REPORT_CHANNEL_ID = int(os.getenv("REPORT_CHANNEL_ID", "1547191196370083871"))
REPORT_CATEGORY_ID = int(os.getenv("REPORT_CATEGORY_ID", "1534053104440578048"))
...
def next_report_number() -> int:
    data = load_json(REPORTS_FILE, {"last_number": 0})
    number = int(data.get("last_number", 0)) + 1
    data["last_number"] = number
    save_json(REPORTS_FILE, data)
    return number


def report_review_layout(title: str, reporter_id: int, target_id: int, reason: str, *,
                         moderator_id: int | None = None, reject_reason: str | None = None,
    ...
    items: list[Any] = [
        ...
        discord.ui.TextDisplay("\n".join(info)),
        discord.ui.Separator(),
        discord.ui.TextDisplay(f"**Причина жалобы:**\n> {reason}"),
    ]
    if reject_reason is not None:
        items.extend([
            discord.ui.Separator(),
            discord.ui.TextDisplay(f"**Причина отклонения:**\n> {reject_reason}"),
        ])
    if view_items:
        items.append(discord.ui.Separator())
...
class RejectReportModal(discord.ui.Modal, title="Отклонить жалобу"):
    ...
    def __init__(self, review: "ReportReviewView"):
        super().__init__()
        self.review = review

    async def on_submit(self, interaction: discord.Interaction) -> None:
        if self.review.resolved:
            await interaction.response.send_message("Эта жалоба уже рассмотрена.", ephemeral=True)
            return
        self.review.resolved = True
        reason = str(self.reject_reason.value).strip()
        await interaction.response.edit_message(
            view=report_review_layout(
                "Жалоба отклонена",
                self.review.reporter_id,
                self.review.target_id,
                self.review.reason,
                moderator_id=interaction.user.id,
                reject_reason=reason,
                color=REPORT_COLOR_REJECTED,
            ),
            allowed_mentions=discord.AllowedMentions.none(),
        )


class ReportReviewView(discord.ui.LayoutView):
    def __init__(self, reporter_id: int, target_id: int, reason: str):
        super().__init__(timeout=None)
        ...
        accept = discord.ui.Button(label="Принять", style=discord.ButtonStyle.success)
        reject = discord.ui.Button(label="Отклонить", style=discord.ButtonStyle.danger)
        accept.callback = self.accept_report
        reject.callback = self.reject_report
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay("-# Жалобы"),
            ...
            discord.ui.TextDisplay(f"**Причина жалобы:**\n> {reason}"),
            discord.ui.Separator(),
            discord.ui.ActionRow(accept, reject),
            accent_color=REPORT_COLOR_OPEN,
        ))

    async def reject_report(self, interaction: discord.Interaction) -> None:
        if self.resolved:
            await interaction.response.send_message("Эта жалоба уже рассмотрена.", ephemeral=True)
            return
        await interaction.response.send_modal(RejectReportModal(self))

    async def accept_report(self, interaction: discord.Interaction) -> None:
        if self.resolved:
            await interaction.response.send_message("Эта жалоба уже рассмотрена.", ephemeral=True)
            return
        if interaction.guild is None or not isinstance(interaction.user, discord.Member):
            await interaction.response.send_message("Недоступно вне сервера.", ephemeral=True)
            return
        ...
        if not isinstance(category, discord.CategoryChannel):
            self.resolved = False
            await interaction.response.send_message("Категория жалоб не найдена.", ephemeral=True)
            return
        ...
        overwrites = {
            ...
            moderator: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        if reporter is not None:
            overwrites[reporter] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)
        if target is not None:
            overwrites[target] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)

        try:
            channel = await guild.create_text_channel(
                f"жалоба-{number}", category=category, overwrites=overwrites,
                reason=f"Жалоба #{number}, принята {moderator}",
            )
        except (discord.Forbidden, discord.HTTPException) as error:
            self.resolved = False
            await interaction.response.send_message(f"Не удалось создать канал жалобы: {error}", ephemeral=True)
            return
        ...
        await interaction.response.edit_message(
            view=report_review_layout(
                ...
                color=REPORT_COLOR_ACCEPTED,
            ),
            allowed_mentions=discord.AllowedMentions.none(),
        )

        case_view = ReportCaseView(
...
class ReportCaseView(discord.ui.LayoutView):
    def __init__(self, number: int, reporter_id: int, target_id: int, moderator_id: int,
                 reason: str, review_channel_id: int, review_message_id: int):
        super().__init__(timeout=None)
        self.number = number
        self.reporter_id = reporter_id
        self.target_id = target_id
        ...
        decide = discord.ui.Button(label="Вынести решение", style=discord.ButtonStyle.success)
        close = discord.ui.Button(label="Закрыть жалобу", style=discord.ButtonStyle.danger)
        decide.callback = self.decide
        close.callback = self.close_no_action
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay("-# Жалобы"),
            discord.ui.TextDisplay(f"## Жалоба #{number}"),
            discord.ui.Separator(),
            discord.ui.TextDisplay(
            ...
            discord.ui.Separator(),
            discord.ui.ActionRow(decide, close),
            accent_color=REPORT_COLOR_ACCEPTED,
        ))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.moderator_id:
            await interaction.response.send_message("Решение по жалобе принимает модератор, который её принял.", ephemeral=True)
            return False
        return True

    async def decide(self, interaction: discord.Interaction) -> None:
        await interaction.response.send_message(
            view=PunishmentView(self), ephemeral=True,
            allowed_mentions=discord.AllowedMentions.none(),
        )

    async def close_no_action(self, interaction: discord.Interaction) -> None:
        await self.finish(interaction, "Без наказания")

    async def finish(self, interaction: discord.Interaction, decision: str) -> None:
        if self.closed:
            if not interaction.response.is_done():
                await interaction.response.send_message("Жалоба уже закрыта.", ephemeral=True)
            return
        self.closed = True
        guild = interaction.guild
        if guild is None:
        ...
        if isinstance(review_channel, discord.TextChannel):
            try:
                ...
                await msg.edit(view=log_layout("Жалобы", "Жалоба рассмотрена", body),
                               allowed_mentions=discord.AllowedMentions.none())
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                pass
        if not interaction.response.is_done():
            await interaction.response.send_message("Жалоба закрыта. Канал будет удалён.", ephemeral=True)
        await asyncio.sleep(2)
        if isinstance(interaction.channel, discord.TextChannel):
            try:
                await interaction.channel.delete(reason=f"Жалоба #{self.number} рассмотрена")
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                pass


class PunishmentView(discord.ui.LayoutView):
    def __init__(self, case: ReportCaseView):
        super().__init__(timeout=120)
        self.case = case
        ban = discord.ui.Button(label="Бан", style=discord.ButtonStyle.danger)
        kick = discord.ui.Button(label="Кик", style=discord.ButtonStyle.danger)
        timeout = discord.ui.Button(label="Тайм-аут 1 час", style=discord.ButtonStyle.secondary)
        none = discord.ui.Button(label="Без наказания", style=discord.ButtonStyle.secondary)
        ban.callback = self.ban
        kick.callback = self.kick
        timeout.callback = self.timeout_member
        none.callback = self.none
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay("-# Жалобы"),
            discord.ui.TextDisplay("## Выбрать наказание для пользователя"),
            discord.ui.Separator(),
            discord.ui.TextDisplay(f"Выберите наказание для <@{case.target_id}>."),
            discord.ui.Separator(),
            discord.ui.ActionRow(ban, kick, timeout, none),
            accent_color=COLOR,
        ))

    async def _target(self, interaction: discord.Interaction) -> discord.Member | None:
        if interaction.user.id != self.case.moderator_id:
            await interaction.response.send_message("Эта панель доступна только модератору жалобы.", ephemeral=True)
            return None
        if interaction.guild is None:
            return None
        member = interaction.guild.get_member(self.case.target_id)
        if member is None:
    ...
    async def ban(self, interaction: discord.Interaction) -> None:
        ...
        try:
            await member.ban(reason=f"Жалоба #{self.case.number}, модератор {interaction.user}")
        except (discord.Forbidden, discord.HTTPException) as error:
            await interaction.response.send_message(f"Не удалось забанить пользователя: {error}", ephemeral=True)
            return
        await self.case.finish(interaction, "Бан")

    async def kick(self, interaction: discord.Interaction) -> None:
        member = await self._target(interaction)
        if member is None: return
        try:
            await member.kick(reason=f"Жалоба #{self.case.number}, модератор {interaction.user}")
        except (discord.Forbidden, discord.HTTPException) as error:
            await interaction.response.send_message(f"Не удалось кикнуть пользователя: {error}", ephemeral=True)
            return
        await self.case.finish(interaction, "Кик")

    async def timeout_member(self, interaction: discord.Interaction) -> None:
        member = await self._target(interaction)
        if member is None: return
        try:
            await member.timeout(timedelta(hours=1), reason=f"Жалоба #{self.case.number}, модератор {interaction.user}")
        except (discord.Forbidden, discord.HTTPException) as error:
            await interaction.response.send_message(f"Не удалось выдать тайм-аут: {error}", ephemeral=True)
            return
        await self.case.finish(interaction, "Тайм-аут на 1 час")

    async def none(self, interaction: discord.Interaction) -> None:
        if interaction.user.id != self.case.moderator_id:
            await interaction.response.send_message("Эта панель доступна только модератору жалобы.", ephemeral=True)
            return
        await self.case.finish(interaction, "Без наказания")
...
class ReportModal(discord.ui.Modal, title="Отправить жалобу"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        target_mention = target.mention if target is not None else f"<@{self.target_id}>"
        reason = str(self.reason.value).strip()
        review = ReportReviewView(interaction.user.id, self.target_id, reason)
        message = await send_log_to(interaction.guild, review, REPORT_CHANNEL_ID)
        if message is None:
            await interaction.response.send_message("Не удалось отправить жалобу.", ephemeral=True)
            return
        await interaction.response.send_message(
            view=log_layout(
                "Жалобы",
                "Жалоба отправлена",
                f"Жалоба на пользователя {target_mention}, успешно отправлена.",
            ),
            ephemeral=True,
            allowed_mentions=discord.AllowedMentions.none(),
        )
...
@discord.app_commands.rename(target="пользователь")
@discord.app_commands.describe(target="Пользователь, на которого Вы хотите пожаловаться.")
async def report_command(interaction: discord.Interaction, target: discord.Member) -> None:
    if interaction.guild is None or not isinstance(interaction.user, discord.Member):
        await interaction.response.send_message("Команда доступна только на сервере.", ephemeral=True)
        return
    if target.id == interaction.user.id:
        await interaction.response.send_message("Вы не можете пожаловаться на самого себя.", ephemeral=True)
        return
    if target.bot:
        await interaction.response.send_message("Вы не можете пожаловаться на бота.", ephemeral=True)
        return
